...
const projects = [
  { number: '01', title: '嵌入式智能终端', type: 'Embedded / Firmware', text: '从驱动到功能验证，练习让软硬件稳定协同。' },
  { number: '02', title: 'OpenCV 人脸识别', type: 'C++ / Qt / Vision', text: '把图像处理、桌面交互和工程实现连在一起。' },
  { number: '03', title: '下一件作品', type: 'In progress', text: '正在整理中，之后分享过程、技术栈与结果。' },
];
...
function App() {
  ...
  return <div className="app">
    ...
    <main id="top">
      ...
      <section className="projects section" id="projects"><div className="section-meta"><span>02</span><span>SELECTED WORK</span></div><div className="section-title"><h2>精选项目</h2><p>记录我如何学习、拆解并完成一个工程。</p></div><div className="project-list">{projects.map((project) => <article className="project-card" key={project.number}><div className="project-visual"><span>{project.number}</span><div className="visual-center">{project.number === '01' ? '01' : project.number === '02' ? '◎' : '＋'}</div></div><div className="project-info"><span className="project-type">{project.type}</span><h3>{project.title}</h3><p>{project.text}</p><a href="#contact" aria-label={`了解${project.title}`}>了解项目 <ArrowUpRight size={17} /></a></div></article>)}</div></section>
